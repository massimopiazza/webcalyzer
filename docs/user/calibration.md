...
| **Field slots** | The five canonical slots plus any custom quantity slots enabled from the library. |
...
Click **Split here** to create a new segment at the current frame. The split frame becomes the first frame of the next segment. The new segment copies the active segment's visible slot list, enabled slots, and boxes so you can adjust only the fields that move or disappear. Use **Crop start** and **Crop end** to define the portion of the source video that should be processed.
...
Click **Add quantity** to choose from the quantity library. The selected quantity is enabled across every segment and copied into the profile as a `custom_telemetry_quantities` snapshot.

Custom slots use generated names such as `custom_acceleration`. Draw boxes for them the same way as canonical fields. You can remove a custom slot from only the active segment, or remove it from all segments, without deleting the quantity from the library. Quantity library management is covered in [quantities](quantities.md).

### Edit slot metadata

The canonical slot names, types, and stages are fixed:
...
Custom slots use `kind: custom`, have no stage, and reference a quantity ID. Their display unit and aliases come from the embedded quantity snapshot.
