...
Unit identification is alias-driven. Webcalyzer first tries exact unit aliases, then fuzzy unit recovery for OCR mistakes, then configured inference rules. During OCR, a numeric measurement with an unrecognized unit-like token triggers a calibrated crop retry before series resolution. For each measurement series, the resolver also looks for a dominant explicit unit and can use that context to recover an isolated missing or malformed unit label. If the evidence is weak or all options imply an implausible jump, it leaves a gap in the clean telemetry instead of forcing a value.
